...
module.exports = function(url, options, response, callback) {
  ...
  response.html = response.html +
    '<script>' +
    '(function() { function reparse() { console.log("reparse"); window.FB ? setTimeout(function() { FB.XFBML.parse(); }, 100) : setTimeout(reparse, 100); } reparse(); })();' +
    '<\/script>';
    
  //FB doesn't include a thumbnail image URL, pull it manually from their graph API
  if (response.url) {
    var matches = response.url.match(/\/videos\/[^\d]*(\d+)/);
    if (matches) {
